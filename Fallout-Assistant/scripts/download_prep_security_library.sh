...
manifest="$project_root/library/prep-security-sources.tsv"
pdf_root="$project_root/library/pdfs/prep-security"
text_root="$project_root/library/text/prep-security"
scratch_root="$project_root/tmp/prep-security"
catalog="$project_root/library/catalog.tsv"
provenance="$project_root/library/prep-security-provenance.tsv"
...
provenance_tmp="$scratch_root/prep-security-provenance.tsv"
: > "$generated_catalog"
...
while IFS=$'\t' read -r id filename title category source_url publisher safety_note rights_note; do
    ...
    if [[ -f "$destination" && "$(LC_ALL=C head -c 5 "$destination")" == "%PDF-" ]]; then
        ...
        curl -fL --http1.1 --retry 4 --retry-all-errors --retry-delay 2 \
            --connect-timeout 20 --max-time 900 "$source_url" -o "$temporary"
        [[ "$(LC_ALL=C head -c 5 "$temporary")" == "%PDF-" ]] || {
            echo "Downloaded file is not a PDF: $title" >&2; exit 4;
        }
        mv "$temporary" "$destination"
    fi
    pages="$(pdfinfo "$destination" | awk -F: '$1 == "Pages" {gsub(/[[:space:]]/, "", $2); print $2; exit}')"
    [[ "$pages" =~ ^[0-9]+$ ]] || { echo "Could not verify page count: $title" >&2; exit 5; }
    pdftotext -layout "$destination" "$text_root/$id.txt"
    [[ -s "$text_root/$id.txt" ]] || { echo "No extractable text found: $title" >&2; exit 6; }
    sha256="$(shasum -a 256 "$destination" | awk '{print $1}')"
    bytes="$(wc -c < "$destination" | tr -d ' ')"
    printf '%s\t%s\t%s\t%s\tpdfs/prep-security/%s\ttext/prep-security/%s.txt\n' \
...
echo "Prep/security library ready: $(wc -l < "$generated_catalog" | tr -d ' ') PDFs"
