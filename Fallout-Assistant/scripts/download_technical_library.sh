...
manifest="$project_root/library/technical-sources.tsv"
pdf_root="$project_root/library/pdfs/technical"
text_root="$project_root/library/text/technical"
scratch_root="$project_root/tmp/technical"
catalog="$project_root/library/catalog.tsv"
provenance="$project_root/library/technical-provenance.tsv"
...
provenance_tmp="$scratch_root/technical-provenance.tsv"
: > "$generated_catalog"
awk -F '\t' 'NR > 1 {print $1}' "$manifest" > "$id_list"
printf 'id\tsha256\tbytes\tpages\tdownloaded_on\tsource_url\tpublisher\tsafety_note\n' > "$provenance_tmp"
...
echo "Technical library ready: $(wc -l < "$generated_catalog" | tr -d ' ') PDFs"
